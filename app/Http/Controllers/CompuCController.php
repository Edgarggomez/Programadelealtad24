		...
		if(is_array($request->consumos))
		{
				foreach ($request->consumos as $consumo)
				{
		            ...
		            if($result && $cte)
		            {
                        ...
                        if ($consumoNuevo) {
                            $client = Cliente::find($cte->id_cliente);
                            $movSaldo = new MovimientoSaldo;
                            if (strcasecmp($consumo['tipo'], 'abono') == 0) {
                                $monto = $this->getMonto($result->id_ubicacion, $consumo['fecha_consumo'], $consumo['monto'], $cte->id_cliente);
                            } else {
                                $monto = $consumo['monto'] * -1;
                            }

                            $movSaldo->id_cliente = $cte->id_cliente;
                            $movSaldo->id_tarjeta = $cte->id_tarjeta;
                            $movSaldo->id_ubicacion = $result->id_ubicacion;
                            $movSaldo->id_consumo = $con->id_consumo;
                            $movSaldo->tipo = $consumo["tipo"];
                            $movSaldo->origen = 'tienda';
